...
│   ├── search_expense_reimbursement.py   # 报销复合技能（分类→标准两步链 + 结论 chunk + 域隔离）
...
{
  "steps": [
    {"step_id": 1, "skill": "search_classification", "query": "深圳 地区分类", "depends_on": []},
    {"step_id": 2, "skill": "search_standards", "query": "<step_1_result> 住宿费标准", "depends_on": [1]}
  ]
}
```
...
| `search_expense_reimbursement` | 报销费用查询（复合） | scope 路由 → 分类推断 → 结论 chunk → 表格优先重排 → 反思自检（域隔离） |
...
南昌（B类省会）→ 省会城市名单前置 → LLM 读名单+规则 → "B类"
揭阳（C类其他）→ BM25 命中规则 → LLM 排除法推断 → "C类"
```
...
**解决方案**：参照 `search_classification.py` 的设计，在 `merged` 首位插入 `is_conclusion=True` 的结论 chunk，`_filter_by_source` 对结论 chunk 豁免过滤（通过 `is_conclusion` 字段识别）。Generator 的 `_build_entity_note` 优先读取结论 chunk，直接得知城市分类，无需重推。
...
**解决方案**：在 Skill 中维护 `_PROVINCIAL_CAPITALS` 硬编码名单，构造虚拟 chunk `_PROVINCIAL_CAPITALS_CHUNK` 并**前置**到 `classification_chunks` 首位，确保进入 `llm_classify` 的 top-3 上下文。LLM 读到名单后结合完整分类规则推断，不改变分类逻辑，仅补充事实知识。
