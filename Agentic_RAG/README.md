...
│   ├── search_expense_reimbursement.py   # 报销复合技能（静态规则 → LLM 分类 → 费用标准 + 结论 chunk + 域隔离）
...
| `search_expense_reimbursement` | 报销费用查询（复合） | 静态规则注入 → LLM 分类 → 向量检索费用标准 → 表格优先重排 → 反思自检（域隔离） |

#### 地区分类推断机制

`search_expense_reimbursement` 内部采用**静态规则注入 + LLM 统一判断**方案，不再依赖 BM25 检索分类规则：

```
静态规则（_CHUNK_CHINA + _CHUNK_OVERSEA）
    ↓ 注入境内/境外完整分类规则
    ↓
LLM 统一判断（llm_classify_region）
    ↓ 一次调用同时完成地理范围 + 费用类别判断
    ↓
输出分类结论（如"境内A类"、"境外C类"）
    ↓
...
**解决方案**：改为静态规则注入方案——在 `_CHUNK_CHINA` 中直接列出所有 B 类城市（含省会城市和计划单列市），LLM 无需自行推断省会身份，直接对照名单判断。原先的 `_PROVINCIAL_CAPITALS` 硬编码名单和虚拟 chunk 已移除。
