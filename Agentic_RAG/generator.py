...
21.对于“有哪些 / 支持哪些 / 完整列表 / 列出全部”类问题，只要资料中明确列出了某个格式、模式、端口、能力、参数项或表格条目，就必须在答案中覆盖。可以合并、分组、改写，但不得因为摘要、可读性或主次判断而省略已确认条目；如果某类内容属于补充、可选、扩展、条件支持或特殊模式，也应单独标注其类别和条件，而不是删除。
